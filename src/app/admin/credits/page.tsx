    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="card text-center">
          <p className="text-text-secondary text-sm">Nog geen creditpakketten verkocht.</p>
        </div>
      ) : (
            ...
            return (
              <div key={pkg.id} className={`card ${isExpired ? "opacity-50" : ""}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-text-primary">{pkg.customer?.full_name || "Onbekend"}</p>
                    <p className="text-xs text-text-secondary">
                      Aankoopbedrag: {formatCents(pkg.total_amount_cents)} · Resterend: {formatCents(pkg.remaining_amount_cents)}
                    </p>
                    <p className="text-[10px] text-text-secondary mt-0.5">
                      Verloopt: {format(new Date(pkg.expires_at), "d MMM yyyy", { locale: nl })}
                    </p>
                  </div>
                  <span className={`text-[10px] px-2 py-1 rounded-full font-medium ${isExpired ? "bg-danger/10 text-danger" : "bg-success/10 text-success"}`}>
